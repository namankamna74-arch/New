...
const Checkers: React.FC<{ isMuted: boolean }> = ({ isMuted }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center font-orbitron relative">
            ...
            <button onClick={() => setIsTutorialOpen(true)} className="absolute top-0 right-0 p-2 rounded-full hover:bg-white/10" aria-label="Help"><HelpIcon /></button>
             <h2 className="text-3xl mb-4 h-8">
                {winner ? `${winner.charAt(0).toUpperCase() + winner.slice(1)} Wins!` : `Turn: ${turn.charAt(0).toUpperCase() + winner.slice(1)}`}
            </h2>
            <div className="relative border-4 border-gray-600 rounded-lg w-[512px] h-[512px]">
                {/* Board squares */}
                {Array.from({ length: 64 }).map((_, i) => {
                    ...
                    return (
                        <div key={`square-${r}-${c}`}
                            ...
                            className={`w-16 h-16 absolute transition-colors
                            ${(r + c) % 2 === 0 ? 'bg-gray-400' : 'bg-gray-800'}
                            ${validMovesForSelected.find(m => m.to.row === r && m.to.col === c) ? 'bg-green-500/50 cursor-pointer' : ''}
                            `}
                ...
                {flatPieces.map(piece => piece && (
                     <div key={piece.id}
                        ...
                        className={`w-16 h-16 absolute flex items-center justify-center transition-all duration-300 ease-in-out cursor-pointer
                        ${selected && selected.row === piece.r && selected.col === piece.c ? 'ring-4 ring-yellow-400 z-10 rounded-full' : ''}`}
                        style={{ top: piece.r * 64, left: piece.c * 64 }}
                     >
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center
                            ${piece.player === 'red' ? 'bg-accent border-red-800' : 'bg-gray-900 border-gray-700'}
                            border-4 shadow-lg`}>
                           {piece.isKing && <span className="text-yellow-400 text-2xl">👑</span>}
                        </div>
                    </div>
                ))}
            </div>
